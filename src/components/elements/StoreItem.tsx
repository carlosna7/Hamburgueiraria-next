...
const StoreItem = ({id, name, price, imgUrl }: StoreItemProps) => {
  ...
  return (
    <div className='flex flex-col w-90 border-2 border-black rounded-xl'>
		...
		<img className='md:h-52 sm:h-44 h-32 w-full object-cover rounded-xl bg-orange-200' src={imgUrl} alt="img" />
		...
		<div className='flex flex-col gap-4 items-center px-4 pb-4'>
			{quantity === 0 ? (
				<button className='bg-lime-500 sm:p-2 p-1 rounded-md' onClick={() => increaseCardQuantity(id)}>Add to card</button>
			) : 
			<div className='flex flex-col gap-4 items-center'>
				<div className='flex items-center gap-4'>
					...
					<div>
					<button className='bg-red-500 sm:p-2 p-1 rounded-md' onClick={() => removeFromCart(id)}>Remove</button>
				</div>
